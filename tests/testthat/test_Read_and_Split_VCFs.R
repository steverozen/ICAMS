...
test_that("Test ReadAndSplitVCFs for Strelka SBS VCFs", {
  rlang::with_options(lifecycle_verbosity = "quiet", {
    ...
    file5 <- list.files(
      path = "testdata/Strelka-SBS-GRCh37/",
      full.names = TRUE
    )
    split.vcfs5.1 <- ReadAndSplitStrelkaSBSVCFs(file5)
    split.vcfs5.2 <- ReadAndSplitVCFs(
      file5,
...
test_that("Test ReadAndSplitVCFs for Strelka ID VCFs", {
  rlang::with_options(lifecycle_verbosity = "quiet", {
    ...
    vcfs2.3 <-
      suppressWarnings(lapply(vcfs2.1, FUN = CheckAndRemoveDiscardedVariants))
    ...
    vcfs5.3 <-
      suppressWarnings(lapply(vcfs5.1, FUN = CheckAndRemoveDiscardedVariants))
...
test_that("Test ReadAndSplitVCFs always.merge.SBS", {
  file1 <- "testdata/Strelka-SBS-GRCh37/Strelka.SBS.GRCh37.s1.vcf"
  split.vcfs1.1 <- ReadAndSplitVCFs(
    file1,
    variant.caller = "strelka",
    always.merge.SBS = TRUE
  )
  split.vcfs1.2 <- ReadAndSplitVCFs(
    file1,
...
test_that("Test ReadAndSplitVCFs filter.status argument", {
  ...
  split.vcfs1.1 <- ReadAndSplitVCFs(file1, variant.caller = "strelka")
  ...
  split.vcfs1.3 <- ReadAndSplitVCFs(
    file1,
    variant.caller = "strelka",
    filter.status = "."
  )
