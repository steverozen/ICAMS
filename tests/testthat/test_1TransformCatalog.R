...
test_that("Transform a counts catalog with NULL abundance,
          to counts.signature catalog", {
            ...
            expect_equal(sum(colSums(catSBS96.counts.sig)), 4, tolerance = 4 * eps)
            ...
            catSBS192.counts.sig <- 
              TransformCatalog(catSBS192.counts,
                               target.catalog.type = "counts.signature")
            expect_equal(sum(colSums(catSBS192.counts.sig)), 4, tolerance = 4 * eps)
            expect_error(
            ...
            catSBS1536.counts.sig <- 
              TransformCatalog(catSBS1536.counts,
                               target.catalog.type = "counts.signature")
            expect_equal(sum(colSums(catSBS1536.counts.sig)), 4, tolerance = 4 * eps)
            expect_error(
            ...
            catDBS78.counts.sig <- 
              TransformCatalog(catDBS78.counts,
                               target.catalog.type = "counts.signature")
            expect_equal(sum(colSums(catDBS78.counts.sig)), 4, tolerance = 4 * eps)
            expect_error(
            ...
            catDBS144.counts.sig <- 
              TransformCatalog(catDBS144.counts,
                               target.catalog.type = "counts.signature")
            expect_equal(sum(colSums(catDBS144.counts.sig)), 4, tolerance = 4 * eps)
            expect_error(
            ...
            catDBS136.counts.sig <- 
              TransformCatalog(catDBS136.counts,
                               target.catalog.type = "counts.signature")
            expect_equal(sum(colSums(catDBS136.counts.sig)), 4, tolerance = 4 * eps)
            expect_error(
            ...
            catID.counts.sig <- 
              TransformCatalog(catID.counts,
                               target.catalog.type = "counts.signature")
            expect_equal(sum(colSums(catID.counts.sig)), 35, tolerance = 35 * eps)
            expect_error(
