...
test_that("Transformation of a DBS 144 catalog", {
  ...
  cat.t <- ReadCatalog(
    "testdata/regress.cat.dbs.144.csv",
    ref.genome = "GRCh37",
    region = "transcript",
    catalog.type = "counts"
  )
  ...
  x1 <-
    expect_warning(TransformCatalog(
      cat.t,
  ...
  expect_equal(
    colSums(genome.counts.signature),
    rep(1, 4),
    check.attributes = FALSE,
    tolerance = .Machine$double.eps^0.5
  )

  expect_error(TransformCatalog(
    genome.counts.signature,
    target.ref.genome = "GRCh37",
    target.region = "transcript",
    target.catalog.type = "density"
