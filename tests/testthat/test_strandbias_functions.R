...
test_that("PlotTransBiasDistToTSSToPdf function", {
  skip_if("" == system.file(package = "BSgenome.Hsapiens.1000genomes.hs37d5"))
  stopifnot(requireNamespace("BSgenome.Hsapiens.1000genomes.hs37d5"))
  list.of.vcfs <- ReadAndSplitStrelkaSBSVCFs("testdata/Strelka.SBS.GRCh37.vcf")
  annotated.SBS.vcf <- 
  ...
  vcf3 <- annotated.SBS.vcf[REF == "T",]
  vcf4 <- annotated.SBS.vcf[REF %in% c("A", "T"), ]
  out <- PlotTransBiasDistToTSSToPdf(annotated.SBS.vcf, 
                                     file = file.path(tempdir(), "test.pdf"))
  out1 <- PlotTransBiasDistToTSSToPdf(vcf1, 
                                      file = file.path(tempdir(), "test1.pdf"))
  out2 <- PlotTransBiasDistToTSSToPdf(vcf2, 
                                      file = file.path(tempdir(), "test2.pdf"))
  out3 <- PlotTransBiasDistToTSSToPdf(vcf3, 
                                      file = file.path(tempdir(), "test3.pdf"))
  out4 <- PlotTransBiasDistToTSSToPdf(vcf4, 
                                      file = file.path(tempdir(), "test4.pdf"))
  expect_equal(out, TRUE)
  expect_equal(out1, TRUE)
  expect_equal(out2, TRUE)
...
test_that("PlotTransBiasDistToTSS function", {
  skip_if("" == system.file(package = "BSgenome.Hsapiens.1000genomes.hs37d5"))
  stopifnot(requireNamespace("BSgenome.Hsapiens.1000genomes.hs37d5"))
  list.of.vcfs <- ReadAndSplitStrelkaSBSVCFs("testdata/Strelka.SBS.GRCh37.vcf")
  annotated.SBS.vcf <- 
  ...
  vcf3 <- annotated.SBS.vcf[REF == "T",]
  vcf4 <- annotated.SBS.vcf[REF %in% c("A", "T"), ]
  out <- PlotTransBiasDistToTSS(annotated.SBS.vcf = annotated.SBS.vcf,
                               plot.type = "C>A")
  out1 <- PlotTransBiasDistToTSS(annotated.SBS.vcf = vcf1,
                               plot.type = "C>A")
  out2 <- PlotTransBiasDistToTSS(annotated.SBS.vcf = vcf2,
                                plot.type = "C>A")
  out3 <- PlotTransBiasDistToTSS(annotated.SBS.vcf = vcf3,
                                plot.type = "C>A")
  out4 <- PlotTransBiasDistToTSS(annotated.SBS.vcf = vcf4,
                                plot.type = "C>A")
  expect_equal(out, TRUE)
  expect_equal(out1, TRUE)
  expect_equal(out2, TRUE)
