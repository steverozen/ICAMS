...
test_that("VCFsToCatalogs function for Mutect VCFs", {
  rlang::with_options(lifecycle_verbosity = "quiet", {
    ...
    catalogs2 <- MutectVCFFilesToCatalog(
      files,
      ref.genome = "hg19",
      region = "genome",
      return.annotated.vcfs = TRUE
    )
    catalogs3 <- VCFsToCatalogs(
      files,
      ref.genome = "hg19",
      variant.caller = "mutect",
      region = "genome"
    )
    catalogs4 <- VCFsToCatalogs(
      files,
      ref.genome = "hg19",
      variant.caller = "mutect",
      region = "genome",
...
test_that("VCFsToCatalogs function for Strelka SBS VCFs", {
  rlang::with_options(lifecycle_verbosity = "quiet", {
    ...
    catalogs1 <- StrelkaSBSVCFFilesToCatalog(
      files,
      ref.genome = "hg19",
      region = "genome"
    )
    catalogs2 <- StrelkaSBSVCFFilesToCatalog(
      files,
      ref.genome = "hg19",
      region = "genome",
      return.annotated.vcfs = TRUE
...
test_that("VCFsToCatalogs function for Strelka ID VCFs", {
  rlang::with_options(lifecycle_verbosity = "quiet", {
    ...
    catalogs2 <- StrelkaIDVCFFilesToCatalog(
      files,
      ref.genome = "hg19",
      region = "genome",
      return.annotated.vcfs = TRUE
