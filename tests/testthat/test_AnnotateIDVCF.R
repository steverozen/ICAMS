...
test_that("AnnotateIDVCF function with hg19", {
  ...
  list <- 
    AnnotateIDVCF(id.vcf$df, 
                  ref.genome = 
                    BSgenome.Hsapiens.1000genomes.hs37d5::BSgenome.Hsapiens.1000genomes.hs37d5)
  list1 <- AnnotateIDVCF(id.vcf$df, ref.genome = "GRCh37")
  list2 <- AnnotateIDVCF(id.vcf$df, ref.genome = "hg19")
...
test_that("AnnotateIDVCF with hg38", {
  ...
  list3 <- AnnotateIDVCF(id.vcf$df, 
                       ref.genome = 
                         BSgenome.Hsapiens.UCSC.hg38::BSgenome.Hsapiens.UCSC.hg38)
  list4 <- AnnotateIDVCF(id.vcf$df, ref.genome = "GRCh38")
  list5 <- AnnotateIDVCF(id.vcf$df, ref.genome = "hg38")
