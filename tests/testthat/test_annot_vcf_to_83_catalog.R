...
test_that("annot_vcf_to_83_catalog returns zero catalog for empty input", {
  empty_vcf <- data.frame(
    CHROM = character(), POS = integer(), REF = character(),
    ALT = character(), FILTER = character(),
    COSMIC_83 = character(), R = integer()
  )

  result <- annot_vcf_to_83_catalog(empty_vcf, sample_id = "empty")
