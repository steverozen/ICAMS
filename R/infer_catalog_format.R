...
IsICAMSCatalog <- function(object) {
  ...
  if (!is.null(rownames(object))) {
    ...
    if (all(rownames(object) == ICAMS::catalog.row.order[[type]])) {
      ...
      message(
        "The rownames of the input object do not match the catalog row ",
...
Make96Catalog <- function(object) {
  ...
  stop(
    "96 mutation types, but not an SBS96 catalog in ICAMS/COSMIC/SigProfiler",
...
MakeSBS96CatalogFromICAMSExt <- function(cos) {
  ...
  if (ncol(out) == 1) {
    colnames(out) <- colnames(cos)[3]
  }

  if (!setequal(rownames(out), ICAMS::catalog.row.order$SBS96)) {
    stop(
      "The mutation types in this SBS96 catalog is not in correct ICAMS format",
...
MakeID83ICAMSFromSigProID96 <- function(cos) {
  ...
  headerIDsToBeDeleted <- which(
    headerPrev %in% c("complex", "non_matching", "non-matching", "non.matching")
  )
  ...
  if (!setequal(rownames(out), ICAMS::catalog.row.order$ID)) {
    stop(
      "The mutation types in this ID83 catalog is not in correct ICAMS format",
...
MakeSBS192Catalog <- function(object) {
  ...
  if (
    setequal(c("T", "U"), unique(unlist(object[, 1]))) &&
      "C>A" %in% unlist(object[, 2]) &&
      "TTT" %in% unlist(object[, 3])
  ) {
    # Convert ICAMS / COSMIC csv into ICAMS internal format
    return(MakeSBS192CatalogFromICAMSExt(object))
  }
  stop(
    "192 mutation types, but not an SBS192 catalog in",
...
MakeSBS192CatalogFromICAMSExt <- function(cos) {
  ...
  if (!setequal(rownames(out), ICAMS::catalog.row.order$SBS192)) {
    stop(
      "The mutation types in this SBS192 catalog is not in correct ICAMS format",
...
MakeSBS1536Catalog <- function(object) {
  ...
  stop("1536 mutation types, but not an SBS1536 catalog in", " ICAMS format")
}
...
MakeSBS1536CatalogFromICAMSExt <- function(cos) {
  ...
  if (!setequal(rownames(out), ICAMS::catalog.row.order$SBS1536)) {
    stop(
      "The mutation types in this SBS1536 catalog is not in correct ICAMS format",
...
MakeDBS78Catalog <- function(object) {
  ...
  stop(
    "78 mutation types, but not a DBS78 catalog in",
...
MakeDBS78CatalogFromICAMSExt <- function(cos) {
  ...
  if (!setequal(rownames(out), ICAMS::catalog.row.order$DBS78)) {
    stop(
      "The mutation types in this DBS78 catalog is not in correct ICAMS format",
...
MakeDBS136Catalog <- function(object) {
  ...
  stop("136 mutation types, but not a DBS136 catalog in", " ICAMS format")
}
## Quad
## NNNN
MakeDBS136CatalogFromICAMSExt <- function(cos) {
  ...
  if (!setequal(rownames(out), ICAMS::catalog.row.order$DBS136)) {
    stop(
      "The mutation types in this DBS136 catalog is not in correct ICAMS format",
...
MakeDBS144Catalog <- function(object) {
  ...
  stop("144 mutation types, but not a DBS144 catalog in", " ICAMS format")
}
## Ref Var
## NN > NN
MakeDBS144CatalogFromICAMSExt <- function(cos) {
  ...
  if (!setequal(rownames(out), ICAMS::catalog.row.order$DBS144)) {
    stop(
      "The mutation types in this DBS144 catalog is not in correct ICAMS format",
...
MakeID83Catalog <- function(object) {
  ...
  if (
    setequal(c("DEL", "INS"), unique(unlist(object[, 1]))) &&
  ...
  stop(
    "83 mutation types, but not a ID83 catalog in",
    " ICAMS, COSMIC, or SigProfiler format"
  )
}
...
MakeID166Catalog <- function(cos) {
  ...
  if (!setequal(rn, ICAMS::catalog.row.order$ID166)) {
    msg <-
      paste(
        "The row names are not correct:\n",
        "got",
        paste(rn, collapse = ", "),
        "\nexpected",
        paste(ICAMS::catalog.row.order$ID166, collapse = ", ")
      )
    stop(msg)
  }
  ...
  if (!setequal(rn, ICAMS::catalog.row.order$ID)) {
    msg <-
      paste(
        ...
        paste(rn, collapse = ", "),
        "\nexpected",
        paste(ICAMS::catalog.row.order$ID, collapse = ", ")
      )
    stop(msg)
  }
  rownames(out) <- rn
  ...
  if (!setequal(rn, catalog.row.headers.sp$ID83)) {
    msg <-
      paste(
        ...
        paste(rn, collapse = ", "),
        "\nexpected",
        paste(catalog.row.headers.sp$ID83, collapse = ", ")
      )
    stop(msg)
  }
...
MakeCOMPOSITECatalog <- function(object) {
  ...
  stop("1697 mutation types, but not a COMPOSITE catalog in", " ICAMS format")
}
## NN(Ref) NN(Var)
MakeCOMPOSITECatalogFromICAMSExt <- function(cos) {
  ...
  if (!setequal(rownames(out), ICAMS::catalog.row.order$COMPOSITE)) {
    stop(
      "The mutation types in this COMPOSITE catalog is not in correct ICAMS format",
