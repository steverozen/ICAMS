...
#'
#' A key data type in ICAMS is a "catalog" of mutation counts, of mutation
#' densities, or of mutational signatures. A catalog is one of the following:
#'
#' \enumerate{
...
#' to 1, one column per signature).
#'
#' }
#'
#' If you need to create a catalog from a source other than
#' this package (from a source other than
#' \code{\link{ReadCatalog}}
#' or \code{\link{StrelkaSNSVCFFilesToCatalog}},
#' \code{\link{MutectVCFFilesToCatalog}}, etc.), then
#' you must ensure that the rows are in the expected order and have
#' the expected rownames. See \code{\link{CatalogRowOrder}}
#' for the expected rownames and order of rows.
#'
...
#'
#' \item \code{\link{StrelkaIDVCFFilesToCatalog}}, which creates ID (indel) catalog
...
#' Many functions take a the argument \code{genome}.
#' This can be either
#' \enumerate{
...
#'   \code{\link{BSgenome::available.genomes()}} returns the available
#'   genomes.
#'
...
#'  via \code{\link{devtools::install_github}}. Otherwise
#'  install it and the
#'  necessary genomes (e.g. \code{BSgenome.Hsapiens.1000genomes.hs37d5})
#'  manually according to the Bioconductor
#'  instructions
#'  https://bioconductor.org/packages/release/bioc/html/BSgenome.html.
#'  Genomes other than two human genomes mentioned above are
...
#'
#' \item \code{\link{CatalogRowOrder}} Standard order of row names in a catalog.
#'
