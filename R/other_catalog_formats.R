...
ReadStapleGT96SBS <- function(
  ...
  if (ncol(df) == 1) {
    stop(
      "Only one column after reading ",
      file,
      "; do you need to set sep to something other than \"",
      sep,
      "\"?"
    )
  }
...
ConvertCatalogToSigProfilerFormat <- function(input.catalog, file, sep = "\t") {
  ...
  if (nrow(input.catalog) == 96) {
    ...
    stop(
      "Can only convert SBS96, SBS192, SBS1536, DBS78 and ID ICAMS 
  ...
  if (mutation.type == "SBS96") {
    ...
    sbs192_to_sp <- function(x) {
      ...
      if (substring(x, 2, 2) %in% c("A", "G")) {
        ...
        stop(
          "The row headers should not contain characters other than A/C/G/T\n"
        )
      }
    }
    ...
    new.list <- lapply(row.names(input.catalog), function(x) {
      new <- paste(substring(x, 1, 2), ">", substring(x, 3, 4), sep = "")
    })
    row.names(input.catalog) <- unlist(new.list)
