...
#'
#' @keywords internal
GenerateVCFsFromIndelSimpleFile1 <- function(
  file,
  output.dir,
  max.mc.cores = 1
) {
  ...
  if (ncol(dt) > length(simple.col.names)) {
    simple.col.names = c(
      simple.col.names,
      paste0("X", (1 + length(simple.col.names)):ncol(dt))
    )
  }
  ...
  out <- parallel::mclapply(
    ...
    FUN = function(x) {
      ...
      if (length(del.idx) > 0) {
        ...
        if (length(idx.to.remove) > 0) {
          message(
            "Remove deletion variants whose CHROM and POS are beyond the boundaries of ref.genome"
          )
      ...
      if (length(ins.idx) > 0) {
        ...
        if (length(idx.to.remove) > 0) {
          message(
            "Remove insertion variants whose CHROM and POS are beyond the boundaries of ref.genome"
          )
          print(dt1[idx.to.remove.in.dt, ])
          dt1 <- dt1[-idx.to.remove.in.dt, ]
        } else {
      ...
      cancertype = make.names(unique(dt1$CancerType))
      vcf.name <- paste0(
        cancertype,
        ".",
        sample.id,
      ...
      data.table::fwrite(
        list("##VCF reconstructed from simple file"),
    ...
    mc.cores = AdjustNumberOfCores(max.mc.cores)
  ) # end of mclapply for sample id
}
